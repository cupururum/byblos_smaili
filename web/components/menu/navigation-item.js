...
function NavigationItem(props) {
    ...
    console.log(activeSection, title)
    return (
        <>
            { title.toLowerCase() == activeSection ? 
                <div className="relative w-84 "> 
                    <button className="group w-80 h-16 text-menuGroups text-left bg-romance outline-none font-bold text-soft-amber">
                        <p className="border-b w-68 h-16 ml-6 pt-1 border-white-coffee">{props.title}</p>
        ...
        : 
                <div className="w-84  "> 
                    <button className="w-80 h-16 text-menuGroups text-left hover:bg-romance">
                        <p className="border-b w-68 h-16 ml-6 pt-1 border-white-coffee"><Link href={`/menu/${slug}`}>{props.title}</Link></p>
                    </button>               
                </div>
